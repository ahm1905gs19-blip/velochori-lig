...
.digital-scoreboard {
    ...
    font-size: 1.2rem; padding: 6px 15px; border-radius: 8px; min-width: 100px;
    text-align: center; border: 1px solid #334155; margin: 0 30px;
}
...
# --- 2. VERİ VE BAŞLANGIÇ AYARLARI ---
# 11. Hafta skoru (Billispor 16 - 15 Prospor) varsayılan olarak ekleniyor
...
with st.sidebar:
    ...
    with st.form("admin_form"):
        ...
        # Otomatik eşleşme mantığı (Tek haftalar Billi Ev, Çift haftalar Prospor Ev)
    ...
    if st.button("Ligi Sıfırla (Sadece Başlangıca Dön)"):
...
with tab2:
    ...
    for i in range(10):
        ...
        # Maç durumu belirleme
        if res:
            status = '● BİTTİ'; score = f'{res["EvSkor"]} - {res["DepSkor"]}'
        else:
            status = f'🕒 18:30'; score = 'VS'
