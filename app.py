...
/* SENİN İSTEDİĞİN DETAYLI PUAN TABLOSU STİLİ */
...
if 'matches' not in st.session_state:
    # 11. Hafta skoru (Senin 16-15 galibiyetin)
    st.session_state.matches = {
        11: {"Ev": "Billispor", "EvSkor": 16, "Dep": "Prospor", "DepSkor": 15}
    }

def get_live_stats():
    # 10 maçlık baz veriler
    data = {
...
with tab1:
    df = get_live_stats()
    # İSTEDİĞİN DETAYLI TABLO (O-G-B-M-AG-YG-AV-P)
...
with tab2:
    ...
    # 11. Hafta (Dün), 12. Hafta Haftaya Pazar (05.04.2026), sonra her Pazar
    for i in range(10):
        w = 11 + i
        if w == 11:
            m_date = datetime.date(2026, 3, 28) # 11. Hafta (Bugün/Dün)
        else:
            # 12. Hafta haftaya Pazar, devamı da birer hafta arayla Pazar
            m_date = datetime.date(2026, 4, 5) + datetime.timedelta(weeks=i-1)
