...
.digital-scoreboard { background: #273142; color: #00ff85; font-family: 'JetBrains Mono', monospace; font-size: 1.1rem; padding: 6px 15px; border-radius: 8px; min-width: 95px; text-align: center; border: 1px solid #334155; margin: 0 35px; box-shadow: inset 0 0 10px #000; }
.team-name { font-size: 1.05rem; font-weight: 800; color: #1e293b; text-transform: uppercase; flex: 1; }

.league-title { font-size: 36px; font-weight: 900; text-align: center; padding: 20px 0; background: linear-gradient(90deg, #059669, #10b981, #34d399); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
.custom-table { width: 100%; border-collapse: collapse; background: white; border-radius: 12px; overflow: hidden; margin-top: 15px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
.custom-table th { background: #1e293b; color: white; padding: 12px; font-size: 11px; text-align: center; text-transform: uppercase; }
.custom-table td { padding: 12px; text-align: center; border-bottom: 1px solid #f1f5f9; font-weight: 600; font-size: 14px; color: #334155; }
</style>
""", unsafe_allow_html=True)

# --- 2. VERİ VE TARİH YÖNETİMİ ---
if 'matches' not in st.session_state:
    # 11. Hafta skorun otomatik yüklendi
    st.session_state.matches = {11: {"Ev": "Billispor", "EvSkor": 16, "Dep": "Prospor", "DepSkor": 15}}

# TÜRKİYE SAATİ AYARI (2026 yılındayız)
now = datetime.datetime.utcnow() + datetime.timedelta(hours=3)
cur_time_int = now.hour * 100 + now.minute
today_date = now.date()

def get_live_stats():
    # 10. Hafta Sonu Baz Verileri
    data = {
...
with st.sidebar:
    ...
    with st.form("admin_form"):
        ...
        s1 = st.number_input(f"{ev} Skor", 0, 100, 0)
        s2 = st.number_input(f"{dep} Skor", 0, 100, 0)
        if st.form_submit_button("SKORU KAYDET"):
            st.session_state.matches[h] = {"Ev": ev, "EvSkor": s1, "Dep": dep, "DepSkor": s2}
            st.rerun()
    if st.button("Sıfırla (Başlangıca Dön)"):
        st.session_state.matches = {11: {"Ev": "Billispor", "EvSkor": 16, "Dep": "Prospor", "DepSkor": 15}}
        st.rerun()
...
with tab1:
    ...
    for idx, r in df.reset_index(drop=True).iterrows():
        ...
        <div class="team-card {'leader-card' if is_lider else ''}">
            <div style="flex:1;">
                <span style="font-size:9px; font-weight:900; color:{'#b45309' if is_lider else '#64748b'};">{ '🔥 LİDER' if is_lider else f'{idx+1}. SIRADA'}</span>
...
with tab2:
    ...
    for i in range(10): # 11. haftadan 20. haftaya kadar
        ...
        if res:
            status = '● BİTTİ'; score_show = f'{res["EvSkor"]} - {res["DepSkor"]}'
        elif is_today:
            if cur_time_int < start_time:
                status = '<span class="match-day-badge">🔥 MAÇ GÜNÜ</span>'; score_show = '18:30'
            elif start_time <= cur_time_int <= end_time:
                status = '<span class="live-anim">⚽ OYNANIYOR...</span>'; score_show = 'LIVE'
            else:
                status = '🕒 BEKLENİYOR'; score_show = 'VS'
        else:
            status = f'🕒 18:30'; score_show = 'VS'
        ...
        <div class="stadium-card {'match-day-card' if is_today and not res else ''}">
            <div style="background:#f8fafc; padding:8px 15px; display:flex; justify-content:space-between; font-size:11px; font-weight:800;">
                <span style="color:#64748b;">{week_num}. HAFTA</span>
            ...
            <div style="background:#f8fafc; padding:8px; text-align:center; font-size:11px; font-weight:800; border-top:1px solid #f1f5f9;">{status}</div>
        </div>
        """, unsafe_allow_html=True)
