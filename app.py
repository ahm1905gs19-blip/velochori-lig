...
# --- CSS: OPTİMİZE EDİLMİŞ TASARIM (YAZILAR KÜÇÜLTÜLDÜ) ---
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap');
.stApp { background: #f0f4f8; font-family: 'Inter', sans-serif; }

/* BAŞLIK: PARLAYAN AMA DAHA KİBAR */
.league-title {
    font-size: clamp(24px, 7vw, 38px); font-weight: 900; text-align: center;
...
/* PUAN DURUMU KARTLARI (YAZILAR KÜÇÜLTÜLDÜ) */
.team-card { 
...
/* FİKSTÜR VE SAHA */
.match-detail-box { background: white; border-radius: 20px; padding: 20px; border: 1px solid #e2e8f0; }
.score-pill { 
    background: #1e293b; color: #34d399; font-size: 26px; font-weight: 900;
    padding: 6px 25px; border-radius: 12px; min-width: 100px; text-align: center;
}
.football-pitch {
    background: #2d5a27; border-radius: 15px; padding: 20px; border: 2px solid rgba(255,255,255,0.3);
    ...
    background-size: 40px 40px; margin-top: 15px;
}
.player-q {
...
/* TABLO TASARIMI (DAHA SIKIŞIK) */
.custom-table { width: 100%; border-collapse: collapse; background: white; border-radius: 12px; overflow: hidden; margin-top: 15px; }
...
with tab1:
    ...
    for idx, r in df.reset_index(drop=True).iterrows():
        ...
        <div class="team-card {'leader-card' if is_l else ''}">
            <div style="flex:1;">
                <div style="font-size:11px; font-weight:900; color:#94a3b8;">{ "🥇 LİDER" if is_l else f"SIRA {idx+1}"}</div>
                <div style="font-size:20px; font-weight:900; color:#1e293b; margin:2px 0;">{r['Takım'].upper()}</div>
                <div style="display:flex;">{f_html}</div>
            </div>
            <div style="text-align:right;">
                <div style="font-size:32px; font-weight:900; color:#10b981; line-height:1;">{r['P']}<small style="font-size:12px; color:#94a3b8; margin-left:3px;">P</small></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    
    st.markdown("<br><h5 style='margin-bottom:5px;'>📈 DETAYLI TABLO</h5>", unsafe_allow_html=True)
    t_html = f"""<table class="custom-table"><thead><tr><th>TAKIM</th><th>O</th><th>G</th><th>B</th><th>M</th><th>AV</th><th>P</th></tr></thead><tbody>{"".join([f"<tr><td>{r['Takım']}</td><td>{r['O']}</td><td>{r['G']}</td><td>{r['B']}</td><td>{r['M']}</td><td>{r['Av']}</td><td style='color:#10b981; font-weight:900;'>{r['P']}</td></tr>" for _, r in df.iterrows()])}</tbody></table>"""
    st.markdown(t_html, unsafe_allow_html=True)

with tab2:
    for i in range(3): # İlk 3 hafta gösterimi
        w = 11 + i
        ...
        with st.expander(f"🏟️ {w}. HAFTA DETAYI", expanded=(i==0)):
            ...
            <div class="match-detail-box">
                <div style="text-align:center;"><div style="background:#f1f5f9; color:#475569; padding:4px 15px; border-radius:100px; font-size:12px; font-weight:800; display:inline-block; margin-bottom:15px;">📍 Filia Arena</div></div>
                <div style="display:flex; justify-content:space-around; align-items:center; margin-bottom:15px;">
                    <div style="flex:1; text-align:center;"><h4 style="margin:0; font-weight:900;">{ev}</h4></div>
                    <div class="score-pill">{score_val}</div>
                    <div style="flex:1; text-align:center;"><h4 style="margin:0; font-weight:900;">{dep}</h4></div>
                </div>
                <div class="football-pitch">
                    <div style="display:flex; justify-content:space-around; align-items:center;">
                        ...
                        <div style="height:100px; border-left:2px dashed rgba(255,255,255,0.2);"></div>
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                            <div class="player-q">?</div><div class="player-q">?</div><div class="player-q">?</div>
                            <div class="player-q">?</div><div class="player-q">?</div><div class="player-q">?</div>
                        </div>
...
with tab3:
    ...
    st.markdown(f"""
    <div style="background:#0f172a; padding:40px; border-radius:25px; text-align:center; border:3px solid #fbbf24;">
        <div style="font-size:50px;">🏆</div>
        <h2 style="color:white; margin:0; font-weight:900;">{lider['Takım'].upper()}</h2>
        <p style="color:#fbbf24; font-weight:800; letter-spacing:2px; font-size:14px;">ZİRVE SENİN!</p>
        <div style="display:flex; justify-content:center; gap:20px; margin-top:20px;">
            <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:15px; min-width:90px; border:1px solid rgba(255,255,255,0.1);">
                <div style="font-size:26px; font-weight:900; color:white;">{lider['P']}</div>
                <div style="font-size:11px; color:#94a3b8;">PUAN</div>
            </div>
            <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:15px; min-width:90px; border:1px solid rgba(255,255,255,0.1);">
                <div style="font-size:26px; font-weight:900; color:white;">{lider['Av']}</div>
                <div style="font-size:11px; color:#94a3b8;">AVERAJ</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
