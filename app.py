...
/* FİKSTÜR KARTI (KAPALI HALİ) */
.fixture-row {
    display: flex; align-items: center; justify-content: space-between;
    padding: 10px; background: white; border-radius: 10px; border: 1px solid #e2e8f0;
}
.fixture-team { font-weight: 800; font-size: 14px; color: #1e293b; width: 40%; text-align: center; }
.fixture-vs { font-weight: 900; font-size: 16px; color: #10b981; background: #f8fafc; padding: 4px 12px; border-radius: 8px; }
...
with tab1:
    ...
    st.markdown("<h5 style='font-size:13px; margin-top:10px;'>DETAYLI TABLO</h5>", unsafe_allow_html=True)
...
with tab2:
    st.markdown("<h4 style='text-align:center; color:#64748b; font-size:16px;'>GELECEK MAÇLAR</h4>", unsafe_allow_html=True)
    
    for week in range(11, 21):
        ev, dep = ("Billispor", "Prospor") if week % 2 != 0 else ("Prospor", "Billispor")
        arena = "Filia Arena" if week == 11 else "Velochori Arena"
        res = st.session_state.matches.get(week)
        score_val = f"{res['EvS']} - {res['DepS']}" if res else "18:00"
        
        # SofaScore Tarzı Tıklanabilir Accordion
        with st.expander(f"{week}. HAFTA | {ev.upper()} vs {dep.upper()}", expanded=False):
            st.markdown(f"""
            <div style="background: white; padding: 15px; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0;">
                    ...
                    <div style="background: #1e293b; color: #34d399; padding: 5px 20px; border-radius: 10px; font-size: 24px; font-weight: 900;">{score_val}</div>
                    <div style="flex: 1; font-weight: 900; font-size: 18px;">{dep.upper()}</div>
                </div>
                <div class="pitch-container">
                    <div style="color: rgba(255,255,255,0.4); font-size: 10px; font-weight: 800; margin-bottom: 10px;">SAHA DİZİLİŞİ</div>
...
with st.sidebar:
    ...
    with st.form("sc_form"):
        h = st.number_input("Hafta Seç", 11, 20)
        s1 = st.number_input(f"Ev Skor", 0); s2 = st.number_input(f"Dep Skor", 0)
        if st.form_submit_button("SKORU GÜNCELLE"):
