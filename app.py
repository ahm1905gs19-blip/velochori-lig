...
# --- KADRO VERİSİ (Örnek) ---
teams_squads = {
    "Billispor": ["Ahmet (K)", "Mehmet", "Can", "Hüseyin", "Burak", "Efe"],
    "Prospor": ["Murat (K)", "Selim", "Deniz", "Arda", "Volkan", "Gökhan"]
}
...
.player-list { list-style: none; padding: 0; margin: 0; font-size: 13px; }
.player-item { padding: 3px 0; border-bottom: 1px solid rgba(255,255,255,0.1); }
.squad-title { color: #fbbf24; font-weight: 800; font-size: 12px; margin-bottom: 8px; text-transform: uppercase; }

/* STADYUM BİLGİSİ */
.venue-info {
    text-align: center; margin-top: 10px; font-size: 12px; color: #64748b;
    font-weight: 600; background: #f1f5f9; padding: 8px; border-radius: 10px;
}
...
.today-card { border: 2px solid #10b981 !important; background: #f0fdf4 !important; }
.match-container { display: flex; justify-content: space-between; align-items: center; gap: 20px; width: 100%; }
.team-name { font-size: clamp(0.9rem, 2.5vw, 1.2rem); font-weight: 900; color: #0f172a; text-transform: uppercase; }
.digital-scoreboard { 
    background: #0f172a; color: #34d399; font-family: 'JetBrains Mono', monospace; 
    font-size: 1.8rem; padding: 8px 20px; border-radius: 12px; min-width: 100px; text-align: center;
}
.status-pill { font-size: 11px; font-weight: 800; padding: 4px 12px; border-radius: 100px; text-transform: uppercase; margin-top: 10px; display: inline-block; }

/* ŞAMPİYONLUK KARTI VE TABLO STİLLERİ (BOZULMADI) */
...
with tab2:
    ...
    for i in range(10):
        ...
        if res:
            status_text, s_bg, s_color, score_display = '● BİTTİ', '#f1f5f9', '#475569', f'<div>{res["EvSkor"]}</div><div style="margin:0 15px; opacity:0.3;">-</div><div>{res["DepSkor"]}</div>'
        elif is_today:
            status_text, s_bg, s_color, score_display = '🔥 MAÇ GÜNÜ', '#10b981', 'white', 'VS'
        else:
            status_text, s_bg, s_color, score_display = '○ BEKLİYOR', '#f8fafc', '#94a3b8', 'VS'

        # MAÇ KARTI EXPANDER İLE SARMALANDI
        with st.expander(f"{w}. Hafta: {ev_t} vs {dep_t}", expanded=is_today):
            st.markdown(f"""
            <div class="stadium-card {'today-card' if is_today else ''}">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px; font-size:11px; font-weight:800; color:#94a3b8;">
                    <span>{w}. HAFTA • 🕒 18:30</span><span>{tarih_str}</span>
                </div>
                <div class="match-container">
                    <div style="flex:1; text-align:right;"><span class="team-name">{ev_t}</span></div>
                    <div class="digital-scoreboard">{score_display}</div>
                    <div style="flex:1; text-align:left;"><span class="team-name">{dep_t}</span></div>
                </div>
                <div style="text-align:center;"><div class="status-pill" style="background:{s_bg}; color:{s_color};">{status_text}</div></div>
                
                <div class="venue-info">📍 Velochori Arena - Halı Saha C Kapısı | 🏟️ Suni Çim Zemin</div>
                
                <div class="squad-container">
                    <div>
                        <div class="squad-title">{ev_t} İlk 6</div>
                        <ul class="player-list">
                            {"".join([f'<li class="player-item">🏃 {p}</li>' for p in teams_squads.get(ev_t, [])])}
                        </ul>
                    </div>
                    <div style="border-left: 1px dashed rgba(255,255,255,0.2); height: 100px; margin-top: 20px;"></div>
                    <div>
                        <div class="squad-title">{dep_t} İlk 6</div>
                        <ul class="player-list">
                            {"".join([f'<li class="player-item">🏃 {p}</li>' for p in teams_squads.get(dep_t, [])])}
                        </ul>
                    </div>
                </div>
            </div>
...
with tab3:
    df = get_live_stats()
    lider = df.iloc[0]; ikinci = df.iloc[1]
    k_hafta = 20 - lider['O']; g_orani = int((lider['G']/lider['O'])*100) if lider['O']>0 else 0; fark = lider['P'] - ikinci['P']
    champ_html = f"""<div class="championship-card"><div style="font-size: 3rem;">🏆</div><h1 style="margin:10px 0; color:white;">{lider['Takım'].upper()}</h1><p style="color:#fbbf24; font-weight:800;">ŞAMPİYONLUK YOLUNDA LİDER!</p><div style="display:flex; justify-content:center; gap:10px; margin-top:20px; flex-wrap:wrap;"><div class="stat-box"><span class="stat-val">{lider['P']}</span><span class="stat-label">PUAN</span></div><div class="stat-box"><span class="stat-val">{lider['Av']}</span><span class="stat-label">AVERAJ</span></div><div class="stat-box"><span class="stat-val">{k_hafta}</span><span class="stat-label">MAÇ</span></div><div class="stat-box"><span class="stat-val">%{g_orani}</span><span class="stat-label">GALİBİYET</span></div></div><div style="margin-top:25px; padding:15px; background:rgba(255,255,255,0.05); border-radius:15px;"><p style="color:#94a3b8; font-size:0.8rem; margin:0;">RAKİPLE FARK</p><div style="font-size:1.8rem; font-weight:900; color:#10b981;">{fark} PUAN</div></div></div>"""
